...
from train_transformer import make_std_mask
import argparse
...
def test(model, device, dataLoader, criterion):
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataset):
            ...
            src_mask = None
            tgt_mask = make_std_mask(mask, see_future = False)
    ...
    for j,i in enumerate([25,50,75,90]):
        ...
        features = torch.cat([features, static], dim=2)
        src_mask = None
        tgt_mask = make_std_mask(mask, see_future=True)
        output = model(features, x, src_mask, tgt_mask)
...
if __name__ == "__main__":
    ...
    #parser.add_argument("--load_data", type = str, default="../../Insitu_gap_filling_testing_data.csv", help = 'file name of the dataset')
    parser.add_argument("--load_data", type=str, default="../../simulation_testing_data.csv",
    ...
    data = SoilMoistureDataset(opt.load_data, time_varying_features_name = ['x1', 'x2'],
                 static_features_name = ['x3'],
                 transform = 'trim_head')
    BATCH_SIZE = 1
    N = len(data)
